...
<script>
import { decryptLetter } from '../letterCrypto.js'
...
export default {
  ...
  methods: {
    async checkPassword() {
      ...
      try {
        // 密码正确才能解出明文；密码错误时 AES-GCM 校验失败会抛异常
        const paragraphs = await decryptLetter(this.password)
        this.errorMsg = ''
        this.$emit('enter', paragraphs)
      } catch {
